  ...
  <div>
    ...
    <el-table max-height="500" size="small" :data="listData" highlight-current-row v-loading="loading"
     ...
     >
      ...
      <el-table-column  prop="cno" label="会员编号" width="300">
      </el-table-column>
      ...
      <el-table-column  prop="adress" label="默认地址" width="300">
        <template slot-scope="scope">
          <div v-if="scope.row.address=='undefined'">无</div>
      ...
      :filters="[{ text: '退群', value: '0' }, { text: '正常', value: '1' }]"
      :filter-multiple="false"
      column-key="status"
      >
        <template slot-scope="scope">
          <div>
            ...
            <el-tag type="danger" v-else>退群</el-tag>
          </div>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" @closed="afterClose" width="30%" >
      <el-form label-width="120px" :model="editForm" :rules="title=='添加'&&flush?editRules:{}" ref="editForm">
        <el-form-item label="会员编号" v-if="title=='编辑'" prop="eno">
          <el-input clearable size="small" :disabled="title=='编辑'?true:false" v-model="editForm.eno" auto-complete="off" placeholder="请输入会员编号"></el-input>
        </el-form-item>
        <el-form-item label="姓名" prop="name">
          <el-input clearable size="small"  v-model="editForm.name" auto-complete="off" placeholder="请输入客户姓名"></el-input>
        </el-form-item>
        ...
        <el-form-item label="联系电话" prop="tel" >
          <el-input size="small" clearable  v-model="editForm.tel" auto-complete="off" placeholder="请输入联系电话"></el-input>
        </el-form-item>
        <el-form-item label="默认地址" prop="tel" >
          <el-input size="small" clearable  v-model="editForm.address" auto-complete="off" placeholder="请输入客户默认地址"></el-input>
        </el-form-item>
...
// -----------------可动态修改部分
      ...
      excelConfig:{
        list:[],
        tHeader:['会员编号','姓名','性别','关联地址','联系电话','状态','注册时间'],
        tValue:['cno', 'name', 'sex', 'address', 'tel', 'status', 'time'],
        filename: '导出数据'
      },
      
      // 单项搜索关键字集合      // canBlur 开启模糊匹配
      ssKeys: [
        {key: 'cno', label: '会员编号', canBlur: false},
        {key: 'name', label: '姓名', canBlur: true},
...
// -----------------
  ...
  methods: {
    ...
    getdata() {
      ...
      this.totalData = []
      // 模拟数据开始
      getAllData('/getData/members').then(res=>{
        setTimeout(()=>{
          ...
          if(res.code==0){
            ...
            sessionStorage.setItem('customers', JSON.stringify({
    ...
    toOutputExcel: function(){
      ...
      if(this.selectAll){
        // 可修改：全部的任务
        totalData = JSON.parse(sessionStorage.getItem('customers')).totalData ;
      }else{
    ...
    filterChange(filters){
      ...
      let totalData = [...customers.totalData] ;
      let pageparm = {...customers.pageparm} ;
    ...
    handleEdit: function(index, row) {
      ...
      if(uoa.length==0){
        ...
        oldEditData.address = oldEditData.address=='undefined'?'无':oldEditData.address
      ...
      this.$nextTick(()=>{
        this.$refs[editData].validate(valid => {
          if (valid) {
            ...
            let url = this.title == '编辑'? '/getData/updateit':'/getData/insmember'
            updateIt(url, {
              where:{
